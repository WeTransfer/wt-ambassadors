# This file was automatically generated by weform 0.7.2. Do not edit it
# manually.

# NOTE: If you are adding a data source to this file and intend for that data
# source to be discoverable in a plat workspace even though the resource does
# not exist in the plat account, then set the provider attribute value to
# `aws.weform_data_source_provider`.

data "aws_iam_role" "platform" {
  name     = "platform"
  provider = "aws.ecr"
}

data "aws_ami" "base" {
  provider    = "aws.weform_data_source_provider"
  owners      = ["${values(local.aws_accounts)}"]
  most_recent = true

  filter {
    name   = "name"
    values = ["wt_v3_docker*"]
  }

  filter {
    name   = "virtualization-type"
    values = ["hvm"]
  }
}

data "aws_acm_certificate" "wetransfer" {
  provider = "aws.weform_data_source_provider"
  domain   = "${local.wetransfer_tld}"
  statuses = ["ISSUED"]
}

data "aws_route53_zone" "wetransfer" {
  provider     = "aws.weform_data_source_provider"
  name         = "${local.wetransfer_tld}"
  private_zone = false
}

data "aws_acm_certificate" "wetransfernet" {
  provider = "aws.weform_data_source_provider"
  domain   = "${local.wetransfer_net_tld}"
  statuses = ["ISSUED"]
}

data "aws_route53_zone" "wetransfernet" {
  provider     = "aws.weform_data_source_provider"
  name         = "${local.wetransfer_net_tld}"
  private_zone = false
}

data "aws_subnet_ids" "private" {
  vpc_id = "${data.aws_vpc.base.id}"

  tags {
    Zone = "private"
  }
}

data "aws_subnet_ids" "public" {
  vpc_id = "${data.aws_vpc.base.id}"

  tags {
    Zone = "public"
  }
}

data "aws_security_group" "default" {
  vpc_id = "${data.aws_vpc.base.id}"

  # 'default' is a protected name so it will always return
  name = "default"
}

data "aws_security_group" "v2_access" {
  vpc_id = "${data.aws_vpc.base.id}"

  tags {
    tf-v2-access = "true"
  }
}

data "aws_vpc" "base" {
  filter {
    name   = "tag:tfservice"
    values = ["base"]
  }

  filter {
    name   = "tag:tfworkspace"
    values = ["${local.workspace_prefix}"]
  }
}
